"""
DEPRECATED (2026-09-14): This is the V1 CT/PT region-identification algorithm,
kept for reference and reproducibility of results computed before the V2
migration. Do not use for new work — import `TumorRegionIdentifier` from
`utils.tumor_region_identifier` instead, which is now the V2 algorithm.

Key differences from V2:
- PT was defined as a band straddling the tumor boundary symmetrically (both
  into the tumor interior and outward into stroma), via a two-sided distance
  threshold + binary_opening smoothing. V2 defines PT as a purely outward
  margin from the tumor edge, so it never erodes into CT.
- No tissue-vs-background distinction: the margin band could extend into
  non-tissue/background area near the edge of the sampled field. V2
  constrains margin growth to a density-filtered tissue mask (STEP 0).
- No thin-tissue reclassification: a margin band pinched between the tumor
  and the tissue edge could survive as a fragmented sliver. V2 folds such
  cells into CT instead.

See utils/tumor_region_identifier.py for the current algorithm.
"""
import logging
